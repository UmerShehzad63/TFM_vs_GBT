...
For Adult, CatBoost achieves the highest mean Accuracy at **0.8409**, Precision at **0.7330**, F1-score at **0.5936**, and ROC-AUC at **0.8787**.

XGBoost achieves the highest mean Recall at **0.5334**.

TabPFN achieves mean Accuracy of **0.8350**, Precision of **0.7031**, Recall of **0.5238**, F1-score of **0.5838**, and ROC-AUC of **0.8744**.
...
TabPFN achieves the highest mean Accuracy (**0.8980**), Recall (**0.3201**), F1-score (**0.3860**), and ROC-AUC (**0.8716**).

CatBoost achieves the highest Precision (**0.6022**).
...
CatBoost achieves the highest mean Accuracy (**0.7212**), Recall (**0.8957**), F1-score (**0.8182**), and ROC-AUC (**0.7213**).

XGBoost achieves the highest Precision (**0.7621**).

TabPFN achieves Accuracy of **0.7145**, Precision of **0.7550**, Recall of **0.8859**, F1-score of **0.8126**, and ROC-AUC of **0.7146**.
...
Of these 45 comparisons, **34 were statistically significant at the 0.05 level** according to the Wilcoxon signed-rank test.
...
For ROC-AUC, TabPFN exceeds XGBoost by **0.0280** and LightGBM by **0.0521**. Both differences are statistically significant and have large rank-biserial effect sizes.

However, TabPFN performs significantly worse than CatBoost in Accuracy, Precision, F1-score, and ROC-AUC. The ROC-AUC difference is **-0.0043** in favour of CatBoost.
...
The ROC-AUC difference is **0.0571** relative to XGBoost and **0.0601** relative to LightGBM. Both differences are statistically significant and have very large rank-biserial effect sizes.

TabPFN also significantly outperforms CatBoost in Accuracy, Recall, F1-score, and ROC-AUC.

For ROC-AUC, TabPFN exceeds CatBoost by **0.0135**. For F1-score, the difference is **0.0396**.
...
## 4.4 Performance Across Training-Data Regimes
...
In the small-data regime, TabPFN performs better than XGBoost and LightGBM on Accuracy, Precision, F1-score, and ROC-AUC. For example, its mean ROC-AUC is **0.7993**, compared with **0.7304** for XGBoost and **0.6419** for LightGBM.

However, CatBoost remains competitive, with a mean small-regime ROC-AUC of **0.8015**.

In the medium regime, TabPFN again exceeds XGBoost and LightGBM on most metrics. Its mean ROC-AUC is **0.8900**, compared with **0.8629** for XGBoost and **0.8589** for LightGBM.

At large training sizes, the differences become smaller and the tree-based models become increasingly competitive. CatBoost achieves higher mean Accuracy, Recall, F1-score, and ROC-AUC than TabPFN in this regime. For ROC-AUC, CatBoost reaches **0.9182** compared with **0.9116** for TabPFN.
...
In the small regime, TabPFN achieves a mean ROC-AUC of **0.7520**, compared with **0.6437** for XGBoost and **0.6195** for LightGBM.

In the medium regime, TabPFN achieves a mean ROC-AUC of **0.8907**, compared with **0.8364** for XGBoost and **0.8388** for LightGBM.

In the large regime, the advantage remains visible. TabPFN reaches a mean ROC-AUC of **0.9339**, compared with **0.9060** for XGBoost, **0.9131** for LightGBM, and **0.9225** for CatBoost.

A similar pattern is observed for F1-score. In the large regime, TabPFN achieves **0.5521**, compared with **0.4963** for XGBoost, **0.4957** for LightGBM, and **0.5077** for CatBoost.
...
In the small regime, TabPFN achieves a mean ROC-AUC of **0.6767**, compared with **0.6376** for XGBoost and **0.5814** for LightGBM. CatBoost achieves **0.6922**, making it the strongest model among the four for this metric in this regime.

In the medium regime, TabPFN reaches a mean ROC-AUC of **0.7714**, compared with **0.7292** for XGBoost, **0.7273** for LightGBM, and **0.7651** for CatBoost.

For F1-score in the medium regime, TabPFN achieves **0.8191**, while CatBoost achieves **0.8245**.
...
### Overall Training-Regime Pattern
...
- **Adult:** TabPFN is competitive in smaller regimes but CatBoost becomes stronger at larger training sizes.
- **Bank Marketing:** TabPFN maintains an advantage across small, medium, and large regimes.
- **Credit-G:** TabPFN remains competitive with the tree models, while CatBoost remains particularly strong.

Thus, the effect of training-data size is **dataset-dependent rather than universal**.
...
For Adult, mean training time is **0.174 seconds** for XGBoost, **0.189 seconds** for LightGBM, **0.558 seconds** for CatBoost, and **0.679 seconds** for TabPFN.

For Bank Marketing, the corresponding mean training times are **0.212 seconds**, **0.205 seconds**, **0.625 seconds**, and **0.685 seconds**.

For Credit-G, mean training times are **0.065 seconds** for XGBoost, **0.052 seconds** for LightGBM, **0.351 seconds** for CatBoost, and **0.659 seconds** for TabPFN.
...
On Adult, mean prediction time is approximately **0.062 seconds** for XGBoost, **0.099 seconds** for LightGBM, **0.049 seconds** for CatBoost, and **9.036 seconds** for TabPFN.

On Bank Marketing, the corresponding values are **0.046 seconds**, **0.091 seconds**, **0.042 seconds**, and **8.946 seconds**.

On Credit-G, prediction time is approximately **0.010 seconds** for XGBoost, **0.011 seconds** for LightGBM, **0.011 seconds** for CatBoost, and **0.667 seconds** for TabPFN.
...
First, **Bank Marketing provides the clearest evidence in favour of TabPFN**. TabPFN achieves the highest aggregate Accuracy, Recall, F1-score, and ROC-AUC and significantly outperforms the tree-based models on most corresponding paired comparisons. Its advantage is also visible across small, medium, and large training-data regimes.

Second, **Adult demonstrates that the relative advantage of TabPFN depends on the baseline and training regime**. TabPFN performs strongly against XGBoost and LightGBM, particularly in smaller and medium regimes, but CatBoost achieves stronger aggregate performance and becomes particularly competitive at larger training sizes.

Third, **Credit-G demonstrates that strong conventional tree-based performance remains possible on smaller tabular datasets**. TabPFN outperforms XGBoost and LightGBM on several metrics, but CatBoost remains closely matched or superior across most aggregate measures.

The statistical analysis further shows that 34 of the 45 TabPFN-versus-tree comparisons are statistically significant. However, statistical significance is not uniformly associated with a TabPFN advantage: the direction of the difference varies across datasets and baseline models.
...
Overall, the evidence supports the interpretation that **TabPFN is a complementary approach rather than a universal replacement for gradient-boosted tree algorithms**. Its relative effectiveness depends on the dataset, evaluation metric, training-data regime, and computational requirements.